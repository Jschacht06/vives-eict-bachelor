...
/* USER CODE BEGIN WHILE */
  while (1)
  {
    ...
    if(SW1Active())       currentButtons |= 0x01; // Bit 0
    if(SW2Active())       currentButtons |= 0x02; // Bit 1
    if(SW3Active())       currentButtons |= 0x04; // Bit 2
    if(SW4Active())       currentButtons |= 0x08; // Bit 3
    if(UserButtonActive()) currentButtons |= 0x10; // Bit 4
    
    // LED's updaten zodat je lokaal ziet wat er gebeurt
    ByteToLeds(currentButtons);

    //  verzenden naar Firebase
    // we sturen ALLEEN als er iets veranderd is EN als we niet nog wachten op een vorig antwoord
    if((currentButtons != previousButtons) && (isPending == false))
    {
        // Update de vorige status direct, zodat we niet blijven triggeren
        ...
        // JSON Data
        sprintf(httpParameters, "{\"waarde\": %d}", currentButtons);
        
        // URL samenstellen
        sprintf(text, "%s.json?auth=%s", HTTPS_URL_PATCH, HTTPS_DATABASE_SECRET); 
        
        //Request versturen 
        printf("Sending new button state: %d...\r\n", currentButtons);
    ...
    {       
        ...
        {               
            // Controleren op TCP/HTTP
            if(strncmp("+IPD,", receivedText, 5) == 0)
            {
                //de server heeft geantwoord
                isPending = false; 
                
                printf("Server antwoord ontvangen, klaar voor volgende klik.\r\n");

                while(!LookForString1(&circularBuffer, "\r\n"));
                // Wacht op data lijn
                while(circularBuffer.numberOfStringsInBuffer == 0);
                // Lees data lijn
                PopStringFromCircularBuffer(&circularBuffer, receivedText);              
            }
            
            if(strstr(receivedText, "CLOSED") != NULL) {
                 isPending = false;
            }
        }
    }
