...
/* Definitions for runningLightTas */
osThreadId_t runningLightTasHandle;
const osThreadAttr_t runningLightTas_attributes = {
  .name = "runningLightTas",
  .stack_size = 128 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};
...
void RunningLight(void *argument);
...
int main(void)
{
  ...
  /* creation of runningLightTas */
  runningLightTasHandle = osThreadNew(RunningLight, NULL, &runningLightTas_attributes);
...
/* USER CODE BEGIN Header_RunningLight */
/**
* @brief Function implementing the runningLightTas thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_RunningLight */
void RunningLight(void *argument)
{
  /* USER CODE BEGIN RunningLight */
  uint8_t led_pos = 1;
  uint16_t delay_time = 100; // Startwaarde
  uint16_t msg_value = 0;
  ...
  for(;;)
  {
    ...
    if (osMessageQueueGet(adcQueueHandle, &msg_value, NULL, 0) == osOK)
    {
       // Begrens de tijd tussen 10ms en 1023ms
       if (msg_value < 10) msg_value = 10;
       if (msg_value > 1023) msg_value = 1023;
       
       delay_time = msg_value;
    }

    osDelay(delay_time);
  }
  /* USER CODE END RunningLight */
}
...
void AdConversion(void *argument)
{
  ...
  static bool userLedState = false;

  for(;;)
  {
    // AD conversie (raw value 0-4095)
    uint16_t raw_value = GetAdValue(&hadc1);
    
    // Schalen: delen door 4 maakt er 0-1023 van
    uint16_t delay_value = raw_value / 4;

    // Stuur naar queue
    osMessageQueuePut(adcQueueHandle, &delay_value, 0, 0);
...
void UartTx(void *argument)
{
  ...
  for(;;)
  {
    // Willekeurig getal 0 - UINT16_MAX
    uint16_t random_num = (uint16_t)rand();
