...
static volatile bool waitingForRandomDelay = false;
static volatile uint32_t randomDelay = 0;
static volatile uint32_t randomDelayStart = 0;
   ...
   while (1)
    {
        ...
        if (SW1Active())
        {
            if (!timerRunning && !waitingForRandomDelay)
            {
                // Start random delay 
                waitingForRandomDelay = true;
							randomDelay = (rand() % 5000) + 1000; //max 6s
                randomDelayStart = ticks;            }
        ...
        if (SW2Active() && timerRunning)
        {
            timerRunning = false;
            WaitForMs(200); 
        }
        
        // Check if random delay has elapsed
        if (waitingForRandomDelay && (ticks - randomDelayStart >= randomDelay))
        {
            waitingForRandomDelay = false;
            timerRunning = true;
            counter = 0;
