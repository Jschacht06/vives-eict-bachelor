...
int main(void)
{
	...
	while (1)
	{	
		// AD waarde binnen nemen via functie en koppelen aan variabele
		uint16_t adValue = GetAdValue();
		float voltage = (adValue / 4095.0) * 3.3;
			sprintf(text, "AD-waarde: %u, Spanning: %.2f V\r\n", adValue, voltage);
    StringToUsart2(text);
    // interval tussen metingen
    WaitForMs(500);
	}
}
