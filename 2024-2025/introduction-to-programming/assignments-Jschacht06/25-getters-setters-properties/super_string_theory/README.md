...
The `Main()` method already provides a partial demo application. You will also need to refactor this to take the new `Text` property into account instead of the getter and setter. 
...
Expand with the following functionality:
* `Reverse()`: reverses the contens of the string
* `CountCharacter(char character)`: counts the times the give character occurs in the string
* `ToUpperCase()`: changes all characters `a`..`z` to uppercase, the other characters remain the same.
